...
class KVStore:
    ...
    def _trans_list(self):
        if not hasattr(self._local, "trans_list"):
            self._local.trans_list = list()

        return self._local.trans_list

    def begin(self):
        self._trans_list().append({})

    def rollback(self):
        if len(self._trans_list()) == 0:
    ...
    def commit(self):
        ...
        commit_trans = self._trans_list().pop()

        if len(self._trans_list()) == 0:
            self.commit_to_prim(commit_trans)
            return

        last_trans = self._trans_list()[-1]
    ...
    def get(self, key):
        ...
        for i in range(len(self._trans_list())-1, -1, -1):
